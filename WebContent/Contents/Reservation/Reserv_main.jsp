            ...
            <nav class="nav" role="navigation">
              <ul class="nav__list">
                <li>
                  ...
                  <ul class="group-list">
                    <li><a href="/ReservServlet?command=reserv_main&row=1&place=1&outplace=${outplace}">서울 / 경기 / 인천</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=1&place=2&outplace=${outplace}">강원도</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=1&place=3&outplace=${outplace}">대전 / 충청남도 / 충청북도</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=1&place=4&outplace=${outplace}">광주 / 전라남도 / 전라북도</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=1&place=5&outplace=${outplace}">부산 / 대구 / 경상남도 / 경상북도</a></li>
                    </li>
                ...
                <ul class="group-list">
                  <li>
                    <li><a href="/ReservServlet?command=reserv_main&row=-1&outplace=1&place=${place}">서울 / 경기 / 인천</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=-1&outplace=2&place=${place}">강원도</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=-1&outplace=3&place=${place}">대전 / 충청남도 / 충청북도</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=-1&outplace=4&place=${place}">광주 / 전라남도 / 전라북도</a></li>
                    <li><a href="/ReservServlet?command=reserv_main&row=-1&outplace=5&place=${place}">부산 / 대구 / 경상남도 / 경상북도</a></li>
                  </li>
...
<a style="display:scroll;" href="#">
		...
		<c:if test="${ place!=0 }">  
		<div class="btn_scroll_up" style="top: 700px;left: 1510px;">
		  <a href="/ReservServlet?command=reserv_main&row=-1&outplace=${ outplace }&place=0">
		  	<font color="white">X&nbsp;&nbsp;탑승지 : ${ p_button }</font>
		  </a>
		</div>
		</c:if>  
		<c:if test="${ outplace!=0 }">  
		<div class="btn_scroll_up" style="top: 800px;left: 1510px;">
		  <a href="/ReservServlet?command=reserv_main&row=1&outplace=0&place=${place}">
		  	<font color="white">X&nbsp;&nbsp;도착지 : ${ op_button }</font>
		  </a>
		</div>  
		</c:if>  
        ...
        <c:set var="index" value="${ index+1 }"></c:set>    
            ...
            <td>
              <section class="hoverEffect">
                ...
                <a href = "ReservServlet?command=reserv_view&p_idx=${ mlist.p_idx }&id=${user.id}">
                 <div class="txt"><h3>${ mlist.p_title }</h3>예약인원수 ${ mlist.p_reservpeople }/${ mlist.p_maxpeople }<br>가격 ${ mlist.p_price }원<br>출발일 : ${mlist.p_indate.substring(0,10) }</div>
                </a>
              </section>
            </td>
	            ...
	            <c:if test="${ stat.count%4==0 || index==mlistcnt}">
	               ...
	               <c:forEach var="mlist2" items="${ mlist2 }" begin="${ cnt*4}" end="${ (cnt*4)+3 }" varStatus="stat2">
	                  ...
	                  <td><font size = "4">
						  <p><center>출발지: ${ mlist2.p_in }  </p>                  
	                      <p><center>도착지: ${ mlist2.p_out }  </p>
